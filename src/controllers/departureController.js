...
// Verificar si la partida está en uso en detalles de notas de débito
const partidaIsInUseInDebitNotes = async (cod_par) => {
    const [rows] = await db.query('SELECT COUNT(*) as count FROM drn_ren WHERE par_drn = ?', [cod_par]);
    return rows[0].count > 0;
};

// Verificar si la partida está en uso en órdenes de pago
const partidaIsInUseInOrders = async (cod_par) => {
    const [rows] = await db.query('SELECT COUNT(*) as count FROM opg_ren WHERE par_opg = ?', [cod_par]);
    return rows[0].count > 0;
};
...
const deleteDeparture = async (req, res) => {
    const { cod_par } = req.params;
    try {
        // No eliminar si está en uso en detalles de notas de débito
        const inDebitNotes = await partidaIsInUseInDebitNotes(cod_par);
        if (inDebitNotes) {
            return res.status(409).json({ message: 'No se puede eliminar esta Partida porque está siendo utilizada en Detalles de Gasto (Notas de Débito) existentes.' });
        }

        // No eliminar si está en uso en órdenes de pago
        const inOrders = await partidaIsInUseInOrders(cod_par);
        if (inOrders) {
            return res.status(409).json({ message: 'No se puede eliminar esta Partida porque está siendo utilizada en Órdenes de Pago existentes.' });
        }
