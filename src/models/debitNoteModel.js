...
const getDebitNoteModel = async ({ cod_ndb }) => {
    const [rows] = await db.query(`SELECT * FROM ndb_ren WHERE cod_ndb=?`, [cod_ndb]);
    return rows[0];
};
...
const getDebitNoteBudgetModel = async (rnd_ndb, excludedCodNdb = null) => {
    ...
    if (rndIds.length === 0) {
        return { orderAmount, totalNotes: 0, remaining: orderAmount };
    }
    ...
    return { orderAmount, totalNotes: previousSpent + currentSpent, remaining: Math.max(0, remaining) };
};

const createDebitNoteModel = async ({ num_ndb, fec_ndb, ben_ndb, rnd_ndb, con_ndb, mon_ndb, ban_ndb, ref_ndb, pro_ndb, rtc_ndb, tbf_ndb, isl_ndb, sub_ndb }) => {
    const [result] = await db.query(
        'INSERT INTO ndb_ren (num_ndb, fec_ndb, ben_ndb, rnd_ndb, con_ndb, mon_ndb, ban_ndb, ref_ndb, pro_ndb, rtc_ndb, tbf_ndb, isl_ndb, sub_ndb) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)',
        [num_ndb, fec_ndb, ben_ndb, rnd_ndb, con_ndb, mon_ndb, ban_ndb, ref_ndb, pro_ndb, rtc_ndb, tbf_ndb, isl_ndb, sub_ndb]
    );
    return { cod_ndb: result.insertId, num_ndb, fec_ndb, ben_ndb, rnd_ndb, con_ndb, mon_ndb, ban_ndb, ref_ndb, pro_ndb, rtc_ndb, tbf_ndb, isl_ndb, sub_ndb };
};

const updateDebitNoteModel = async (cod_ndb, { num_ndb, fec_ndb, ben_ndb, rnd_ndb, con_ndb, mon_ndb, ban_ndb, ref_ndb, pro_ndb, rtc_ndb, tbf_ndb, isl_ndb, sub_ndb }) => {
    await db.query(
        'UPDATE ndb_ren SET num_ndb=?, fec_ndb=?, ben_ndb=?, rnd_ndb=?, con_ndb=?, mon_ndb=?, ban_ndb=?, ref_ndb=?, pro_ndb=?, rtc_ndb=?, tbf_ndb=?, isl_ndb=?, sub_ndb=? WHERE cod_ndb=?',
        [num_ndb, fec_ndb, ben_ndb, rnd_ndb, con_ndb, mon_ndb, ban_ndb, ref_ndb, pro_ndb, rtc_ndb, tbf_ndb, isl_ndb, sub_ndb, cod_ndb]
    );
    return { cod_ndb, num_ndb, fec_ndb, ben_ndb, rnd_ndb, con_ndb, mon_ndb, ban_ndb, ref_ndb, pro_ndb, rtc_ndb, tbf_ndb, isl_ndb, sub_ndb };
};
